...
| `SendUserMessageUseCase` | Accept a user message and dispatch the agent launcher fire-and-forget; returns immediately. Rejects if a task is already in flight. | `POST /v1/sessions/{session_id}/messages` / `mad_send_message` | Emits `user.message`; then `_run_launcher` emits `session.status_running`, streams `agent.output` (tokens redacted), and `session.status_idle` (exit 0) or `session.error` (non-zero / timeout / rate limit). Launches the external agent in the workspace, then a second post-run auto-sync agent run (issue #8). May emit `agent.conversation_resume_skipped`. |
...
| `build_auto_sync_prompt` | Render the fixed post-run instruction prompt (branch / commit excluding `.claude/settings*.json` / push / open PR). Mad does not interpret the result (hard rule 1). | Internal (used by `_run_launcher`) | None directly; the resulting agent run emits `agent.output`. |
...
| Operation | Description | Input surface | Side effects |
|---|---|---|---|
| `EnqueueTaskUseCase` | Accept a task for a known session; optionally validate the model against the provider catalog. | `POST /v1/sessions/{session_id}/tasks` / `mad_enqueue_task` | Emits `task.queued`. |
