...
| `uvicorn[standard]` | `>=0.29` | The ASGI server. `mad serve` / `make serve` start one uvicorn on a TCP host/port (public app) and a second on a Unix domain socket (the internal hook-ingestion app). Wired in [`entry_points/cli.py`](../../src/mad/entry_points/cli.py). The `[standard]` extra pulls in `uvloop`/`httptools`/`websockets`. |
...
| `httpx` | `>=0.27` | HTTP client. Declared as a direct runtime dependency; in practice it is exercised by FastAPI's `TestClient` and the MCP client transport rather than imported directly under `src/` (no `import httpx` exists in the package today). Listed here because a `mad-bros` consumer inherits it. |
