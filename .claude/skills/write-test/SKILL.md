---
name: write-test
description: Use when writing, adding, or modifying pytest tests in this repo. Loads the seven testing heuristics from docs/testing-heuristics.md and enforces a pre-merge checklist that prevents tautological tests, weak assertions, inline fakes, time-based waits, and missing OpenAPI / SSE contract tests. Triggers on tasks like "add tests for X", "improve coverage", "fix the failing test", or any change under tests/.
---
...
Read `docs/testing-heuristics.md` completely. Internalize the seven rules; they govern every test you write.
...
Apply rules 1–7 from `docs/testing-heuristics.md`:
...
6. New streaming endpoints get an `httpx.AsyncClient` test against the route, not just the helper.
